...
app.controller('mongoController', function($scope, $http){
	...
	$http.get('http://192.168.99.100:8080/api/v2/entries.json')
	...
	siq.getEntry = function(index){
		...
		$http.get('http://192.168.99.100:8080/api/v2/entries/' + id + '.json')
			.then(function(response){
	...
	siq.updateEntry = function(index, subject, content){
		...
		siq.clear();
		$http.put('http://192.168.99.100:8080/api/v2/entries/' + id + '.json', entry)
			.then(function(response){
	...
	siq.deleteEntry = function(index){
		...
		siq.panelNum = -1;
		$http.delete('http://192.168.99.100:8080/api/v2/entries/' + id)
			.then(function(response){
	...
	siq.postEntry = function(subject, content){
		...
		siq.clear();
		console.log(entry);
		$http.post('http://192.168.99.100:8080/api/v2/entries.json', entry)
			.then(function(res){
